...
```{r}
#cols <- as.numeric(Expr.filt$samples$group)
cols <- c('Tumor' = 'red',
          'Liver' = 'blue',
          'Hep3B' = 'green4',
          'HUH7' = 'orange',
...
#plotData <-plotMDS(Expr.filt, col = cols, labels = groups, top = 1000,plot = F)
#rownames(plotData$distance.matrix.squared)
...
```{r}
#pdf('Slide7.pdf', width=15, height=7)
...
```{r}
#pdf('Slide8.pdf', width=16, height=7)
...
#Clust1 <- Expr.filt[,setdiff(colnames(Expr.filt), cluster2)]
#Clust2 <- Expr.filt[,setdiff(colnames(Expr.filt), cluster1)]
```
...
#cols <- as.numeric(Clust1$samples$group)
#plotMDS(Clust1, col = cols, labels = Clust1$samples$group, top = 1000)
```
...
```{r}
#pdf('Slide10.pdf', width=16, height=7)
#ggp_all
#dev.off()
```
...
```{r}
#Clust2 <- createclust(cluster1)
#cols <- as.numeric(Clust2$samples$group)
#plotMDS(Clust2, col = cols, labels = Clust2$samples$group, top = 1000)
```
...
```{r}
#pdf('slides13-16.pdf',width = 8.5, height = 7)
ggp1
ggp2
ggp3
ggp4
...
```{r}
#selpat1 <- gse1@result$Description[gse1@result$NES>0]
#selpat2 <- gse2@result$Description[gse2@result$NES>0]
#selpat3 <- gse3@result$Description[gse3@result$NES>0]
#selpat4 <- gse4@result$Description[gse4@result$NES>0]
dp1 <- dotplot(gse1, showCategory=10, split=".sign") + facet_grid(.~.sign) # Tumor1 vs Liver1
dp2 <- dotplot(gse2, showCategory=10, split=".sign") + facet_grid(.~.sign) # Tumor2 vs Liver2
dp3 <- dotplot(gse3, showCategory=10, split=".sign") + facet_grid(.~.sign) # Tumor1 vs HepG2
dp4 <- dotplot(gse4, showCategory=10, split=".sign") + facet_grid(.~.sign) # Tumor2 vs HepG2

```

```{r}
#pdf('slides13-16gse.pdf', width = 4.5*1.5, height = 5.05*1.5)
dp1
dp2
dp3
dp4
...
```{r}
#pdf('Ch_markers.pdf',width=14, height=7)
...
The rest of this pipeline is not that significant, so it is vary raw.

## KEGG

Пробую с другой библиотекой
...
```{r}
decodance <- function(LivT, keep){
  ids<-bitr(keep, fromType = "ENSEMBL", toType = "ENTREZID", OrgDb=organism)
  dedup_ids <- ids[!duplicated(ids[c("ENSEMBL")]),]
  KL <- LivT$table[dedup_ids$ENSEMBL,'logFC']
  names(KL) <- dedup_ids$ENTREZID
  KL <- sort(KL, decreasing = TRUE)
  return(KL)
  }
...
hb <- results1[["ch2"]]
hu <- results1[["ch3"]]
keep1 <- intersect(hg,intersect(hb,hu))
hg <- results2[["ch1"]]
hb <- results2[["ch2"]]
hu <- results2[["ch3"]]
keep2 <- intersect(hg,intersect(hb,hu))
KL1 <- decodance(LivT1, keeps)
KL2 <- decodance(LivT2, keeps)
```

```{r}
keep1 <- names(KL1)
keep2 <- names(KL2)
keeps <- intersect(keep1,)
```
...
```{r}
kg2 <- gage(exprs = KL2, gsets = kegg.sets.hs, same.dir = TRUE)
```

```{r}
kgp1 <- data.frame(id = rownames(kg1$greater),kg1$greater) %>%
  tibble::as.tibble() %>%
  filter(row_number() <= 5) %>%
  .$id %>%
  as.character()
...
```{r}
kgid1 <- substr(kgp1, 1, 8)
kgid1
```

```{r}
tmp <- sapply(kgid1, function(pid) pathview(gene.data = KL1, pathway.id = pid))
```
